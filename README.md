...
- **All events use the same offset as arcs**. Previously storyboard events, zoom events, etc. would use the old timing offset, while arcs use the new one of 110ms plus the user's arc delay. All map events now use the arcs offset. You no longer need to offset these events to make them match up to arcs. This does offset old maps that were adjusted for the old settings. If this is a big enough issue I'll make a workaround with a new config version.
